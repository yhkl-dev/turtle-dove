...
class ZabbixHostStatusViewset(ViewSet, ListModelMixin):
    ...
    def get_total_host_num(self) -> int:
        """
            返回系统中资源总数
        """
        try:
    ...
    def get_zabbix_monitor_host_num(self) -> int:
        """
        返回zabbix中正常监控的主机数
        """
        try:
    ...
    def get_zabbix_not_monitor_host_num(self) -> int:
        """
...
class ZabbixHostListViewset(ViewSet, ListModelMixin):
    """
        zabbix host List
    """
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def list(self, request, *args, **kwargs):
        queryset = Hosts.objects.filter(status__in=[0, 1]).filter(flags=0).using("zabbix")
        hostinfo = []
        for q in queryset:
            hosts = {
                "hostid": q.hostid,
            ...
            hostinfo.append(hosts)
        return Response(hostinfo, status=status.HTTP_200_OK)
...
class ProductMonitorStatus(object):
    ...
    def initialize(self):
        self.server_queryset = Resource.objects.filter(service_id=self.product)
