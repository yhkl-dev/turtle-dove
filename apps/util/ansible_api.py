...
class ANSRunner(object):
    ...
    # def run_playbook(self, playbook_path: object, extra_vars: object = None) -> object:
...
if __name__ == '__main__':
    ...
    data = {
        'exec_host': ["192.168.75.138", "192.168.75.142"],
        'model_name': 'shell',
        'model_args': 'ls /tmp',
    }
    rbt = ANSRunner()
    # ({'hostname': '192.168.75.138'}, {'hostname': '192.168.75.142'})
    rbt.run_playbook(playbook_path='/tmp/pycharm_project_5/playbook/2019/06/marriage.yaml')
    # rbt.run_playbook(playbook_path='test.yaml')
    result = json.dumps(rbt.get_playbook_result(), indent=4)
    # rbt.run_model(tuple(data.get('exec_host')), data.get('model_name'), data.get('model_args'))
